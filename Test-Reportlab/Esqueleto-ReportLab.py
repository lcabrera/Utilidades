...
# ~ código creado por: monobot.soft@gmail.com (www.alvarezalonso.es/monobotblog)
# ~ code by: monobot.soft@gmail.com (www.alvarezalonso.es/monobotblog)
...
est_1 = ParagraphStyle('', fontName='DejaVu', fontSize=12, alignment=0)
est_2 = ParagraphStyle('', fontName='DejaVu', fontSize=8, alignment=0)
...
##    Cabecera
    ...
    canvas.drawRightString(PAGE_WIDTH - 1 * cm, PAGE_HEIGHT - 1 * cm, 'impresión: %s' % hoy)
    canvas.drawRightString(PAGE_WIDTH - 1 * cm, PAGE_HEIGHT - 1.2 * cm, 'Luis Cabrera Sauco')
...
hoja_moderna = BaseDocTemplate(
    ...
    allowSplitting=1,
    #tittle=None,
    tittle='Jugando con ReportLab',
    # author=None,
    author='Luis Leoncio Cabrera Sauco',
    _pageBreakQuick=1,
    encrypt=None,
    )
...
# ~                  Y   A Ñ A D I M O S  T R E S   P A R R A F O S  Y   D O D   T A B L A S
# ~
...
def go():
    ...
    # os.startfile(nombrefichero)
    # os.system(nombrefichero)
