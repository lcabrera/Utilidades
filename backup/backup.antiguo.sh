...
( /bin/tar -c -f ${BACKUP}/01-cron.tar /var/spool/cron ) > /dev/null 2>&1

# Copia de /root
( /bin/tar -c -f ${BACKUP}/02-root.tar --exclude BACKUP /root ) > /dev/null 2>&1

# Copia de /home
( /bin/tar -c -f ${BACKUP}/03-home.tar /home ) > /dev/null 2>&1

# Copia de /etc
( /bin/tar -c -f ${BACKUP}/04-etc.tar /etc ) > /dev/null 2>&1

# Copia de los log's de apache:
( /bin/tar -c -f ${BACKUP}/05-var_log.tar /var/log ) > /dev/null 2>&1

# Copia de los correos no leidos
( /bin/tar -c -f ${BACKUP}/06-var_mail.tar /var/mail ) > /dev/null 2>&1

# Copia de los directorios web
( /bin/tar -c -f ${BACKUP}/07-var_www.tar /var/www ) > /dev/null 2>&1

# Copia de los servicios
( /bin/tar -c -f ${BACKUP}/08-var_srv.tar /var/srv ) > /dev/null 2>&1
